...
class NewsCollectionListing extends ControllerBase
{
    private $page;
    private $total_pages;
    ...
    public function listNews()
    {
        ...
        if ($this->filter) {
            $query = \Drupal::entityQuery('node')
                ...
                // ->notExists('field_news_link');

            if (!$this->divisions) {
                $query->notExists('field_news_link');
            }
            ...
            if ($this->divisions) {
                ...
                //$query->condition('field_news_participant_division.target_id', $this->divisions, '=');


                //$query->condition('field_news_link.target_id', $this->divisions, '=');
            ...
            //     ->condition('field_news_participant_division.target_id', $this->divisions, '=')
            //     ->condition('field_news_link.target_id', $this->divisions, '=');
          ...
          if ($this->participant) {
            ...
            $query = \Drupal::entityQuery('node')
                ->condition('type', $content_type)  // Указываем тип материала.
                ->condition('status', 1)  // Опубликованные ноды.
                ->notExists('field_news_link') //->condition('field_news_main_flag', 1) // главная новость
                ->sort('field_publ_date', 'DESC') // Сортировка по полю дата публикации
                ->range($begin, $this->output_pages);  // Ограничение на последние 15 записей.
        }
...
//        dpm($node->get('field_news_participant_division')->getValue());
        ...
        if ($node->hasField('field_video_v_shapke_novosti') && !$node->get('field_video_v_shapke_novosti')->isEmpty()) {
            //dpm($node->get('field_video_v_shapke_novosti')->getValue());
